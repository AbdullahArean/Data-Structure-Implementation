...
int MaxDepthOfParanthese(string arr)
{
	int maxdept = 0, i = 0;
	Stack Container;
...
string InfixToPostfix(string s)
{
	...
	for (int i = 0; i < s.length(); i++)
	{
		...
		{
			while (!st.IsEmpty() && (GetPriorityOfOperator(s[i]) <= GetPriorityOfOperator(st.Top())))
			{
				res += st.Top();
				st.Pop();
			}
